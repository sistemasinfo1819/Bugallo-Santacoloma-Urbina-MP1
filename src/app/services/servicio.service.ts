...
export class ServicioService {
  ...
  damePokemon(i: number){
    this.pokemonSeleccionado=this.pokemons[i];
  }

  imgEvolucion(nombreEv:string){
    for(let i=0;i<this.pokemons.length;i++){
      if(this.pokemons[i].name==nombreEv){
        return this.pokemons[i].art_url;
      }
